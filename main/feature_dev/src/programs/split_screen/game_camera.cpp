...
void GameCamera::Update(const seconds dt)
{
	...
	for (std::size_t i = 0; i < cameras_.size(); ++i)
	{
		...
		const float xMove =
			inputManager.GetControllerAxis(i, sdl::ControllerAxes::HORIZONTAL_L);
		const float zMove =
			inputManager.GetControllerAxis(i, sdl::ControllerAxes::VERTICAL_L);
		...
		// Joystick Rotation
		const float xCamera =
			inputManager.GetControllerAxis(i, sdl::ControllerAxes::VERTICAL_R);
		const float yCamera =
			inputManager.GetControllerAxis(i, sdl::ControllerAxes::HORIZONTAL_R);
