...
void AudioManager::Init()
{
	for (std::size_t e = 0; e < components_.size(); ++e)
	{
		...
		if (components_[e].DoPlayOnWakeUp() && !components_[e].IsPlaying())
...
AudioSerializer::AudioSerializer(EntityManager& entityManager, AudioManager& audioManager)
   : ComponentSerializer(entityManager), audioManager_(audioManager)
{}

void AudioSerializer::DrawImGui(Entity entity)
{
...
json AudioSerializer::GetJsonFromComponent(Entity entity) const
{
...
void AudioSerializer::SetComponentFromJson(Entity entity, const json& component)
{
