...
void Hierarchy::DrawImGui()
{
	...
	for (Entity entityIndex = 0; entityIndex < entityManager_.GetEntitiesSize(); entityIndex++)
	{
		...
		if (entityManager_.GetParent(entityIndex) == INVALID_ENTITY &&
...
void Hierarchy::DisplayEntity(Entity entity)
{
	...
	const std::string_view text = entityManager_.GetEntityName(entity);
	...
	for (Entity entityIndex = 0; entityIndex < entityManager_.GetEntitiesSize(); entityIndex++)
	{
		if (entity == entityManager_.GetParent(entityIndex))
		{
			hasChild = true;
			break;
		}
	...
	//Display entity
	const bool nodeOpen = ImGui::TreeNodeEx(text.data(), nodeFlags);
	...
	if (ImGui::BeginDragDropTarget())
	{
		...
		if (payload)    // Get payload
		{
			...
			if (entityManager_.GetParent(child) == entity)
				entityManager_.SetParent(child, INVALID_ENTITY);
			else
				entityManager_.SetParent(child, entity);
		}
